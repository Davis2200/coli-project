...
import { Menu, Search, MessageSquare, User, Settings, LogOut, ChevronDown, Bell } from "lucide-react";
    ...
    <nav className="sticky top-0 z-50 bg-white/70 backdrop-blur-md border-b border-slate-200/50 px-6 py-3">
      <div className="max-w-screen-xl mx-auto flex justify-between items-center">
        ...
        <Link href="/turista/explorar" className="relative w-12 h-12 rounded-full overflow-hidden border-2 border-orange-500 shadow-sm">
          <Image src="/coli-logo.jpeg" alt="Logo" fill className="object-cover" priority sizes="48px" />
        </Link>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            ...
            {/* Despliegue de notificaciones alineado a la derecha del icono */}
            {isNotificationsOpen && (
            ...
            {isDropdownOpen && (
              <>
                ...
                <div className="absolute right-0 mt-3 w-56 bg-white rounded-[24px] shadow-2xl border border-slate-100 py-3 z-20 overflow-hidden animate-in fade-in zoom-in duration-200">
                  ...
                  <Link href="/perfil" onClick={() => setIsDropdownOpen(false)} className="flex items-center gap-3 px-5 py-3 text-sm font-bold text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 transition-colors">
                    <User size={18} /> Ver Perfil
                  </Link>

                  <Link href="/configuracion" onClick={() => setIsDropdownOpen(false)} className="flex items-center gap-3 px-5 py-3 text-sm font-bold text-slate-600 hover:bg-indigo-50 hover:text-indigo-600 transition-colors">
